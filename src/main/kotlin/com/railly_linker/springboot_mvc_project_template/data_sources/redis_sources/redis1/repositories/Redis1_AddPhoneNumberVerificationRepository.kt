...
import com.railly_linker.springboot_mvc_project_template.data_sources.redis_sources.redis1.tables.Redis1_AddPhoneNumberVerification
...
@Component
class Redis1_AddPhoneNumberVerificationRepository(
    ...
    fun saveKeyValue(
        ...
        value: Redis1_AddPhoneNumberVerification,
        expireTimeMs: Long
    ) {
        if (key.trim() == "") {
        ...
        val innerKey = "${Redis1_AddPhoneNumberVerification.TABLE_NAME}:${key}" // 실제 저장되는 키 = 그룹명:키
    ...
    fun findAllKeyValues(): List<KeyValueData> {
        ...
        val keySet: Set<String> = redisTemplate.keys("${Redis1_AddPhoneNumberVerification.TABLE_NAME}:*")
        ...
        for (innerKey in keySet) {
            ...
            val key = innerKey.substring("${Redis1_AddPhoneNumberVerification.TABLE_NAME}:".length) // 키

            // Redis Storage 에 실제로 저장 되는 Value (Json String 형식)
            val innerValue = redisTemplate.opsForValue()[innerKey] ?: continue // 값

            // 외부적으로 사용되는 Value (Json String 을 테이블 객체로 변환)
            val valueObject = Gson().fromJson<Redis1_AddPhoneNumberVerification>(
                innerValue as String, // 해석하려는 json 형식의 String
                object : TypeToken<Redis1_AddPhoneNumberVerification>() {}.type // 파싱할 데이터 스키마 객체 타입
            )
    ...
    fun findKeyValue(
        ...
        // Redis Storage 에 실제로 저장 되는 키 (테이블 이름과 키를 합친 String)
        val innerKey = "${Redis1_AddPhoneNumberVerification.TABLE_NAME}:$key"
    ...
    data class KeyValueData(
        ...
        val value: Redis1_AddPhoneNumberVerification,
